...
#Creates a directed graph with n nodes and randomized edge weights from 1-10
#The graph includes at least one cycle
def create_directed_cycle_graph(n):
    if n < 4:
        raise ValueError("At least 4 nodes needed.")
    
    G = nx.DiGraph()
    dag = nx.gn_graph(n, seed=random.randint(0, 10000))
    for u, v in dag.edges():
        G.add_edge(u, v, weight=random.randint(1, 10))

    cycle_size = random.randint(3, max(3, n // 2))
    cycle_nodes = random.sample(list(G.nodes), cycle_size)

    for i in range(cycle_size):
        G.add_edge(cycle_nodes[i], cycle_nodes[(i + 1) % cycle_size], weight=random.randint(1, 10))

    if not any(nx.simple_cycles(G)):
        raise RuntimeError("No cycle found - Error in Directed Cycle Graph")
    
    return G


#Creates a directed tree graph with n nodes and randomized edge weights from 1-10
def create_directed_tree(n):
    tree = nx.DiGraph()
    nodes = list(range(n))
    ...
    for i in range(1, n):
        ...
        tree.add_edge(nodes[parent], nodes[i], weight=random.randint(1, 10))
    
    return tree


#Creates a directed bipartite graph with n1+n2 nodes and randomized edge weights from 1-10
def create_directed_bipartite_graph(n1, n2):
    top_nodes = range(n1)  
    ...
    for u in top_nodes:
        for v in bottom_nodes:
            if random.choice([True, False]):
                B.add_edge(u, v, weight=random.randint(1, 10))
            else:
                B.add_edge(v, u, weight=random.randint(1, 10))
    
    return B


#Creates a directed graph with n nodes and randomized edge weights from 1-10
def create_directed_planar_graph(n):
    G = nx.DiGraph()
    
    while True:
        undirected_planar = nx.random_geometric_graph(n, radius=0.5)
        if nx.check_planarity(undirected_planar)[0]:
            break
    
    for u, v in undirected_planar.edges():
        if random.choice([True, False]):
            G.add_edge(u, v, weight=random.randint(1, 10))
        else:
            G.add_edge(v, u, weight=random.randint(1, 10))
     
    return G

#Creates a directed star graph with n nodes and randomized edge weights from 1-10
def create_directed_star_graph(n):   
    center_node = 0
    G = nx.DiGraph()
    
    for i in range(1, n):
        if random.choice([True, False]):
            G.add_edge(center_node, i, weight=random.randint(1, 10))
        else:
            G.add_edge(i, center_node, weight=random.randint(1, 10))
    
    return G

#Creates a directed graph with n nodes and randomized edge weights from 1-10
#The graph is strongly connected
def create_directed_strongly_connected_graph(n):
    G = nx.DiGraph()
    G.add_edges_from((i, (i + 1) % n, {'weight': random.randint(1, 10)}) for i in range(n))
    additional_edges = [(random.randint(0, n-1), random.randint(0, n-1), {'weight': random.randint(1, 10)}) for _ in range(n)]
    G.add_edges_from(additional_edges)
    return G

#Creates a directed conected graph with n nodes and randomized edge weights from 1-10
def create_directed_connected_graph(n, m):
...
#Creates a directed hamiltonian graph with n nodes and randomized edge weights from 1-10
def create_directed_hamiltonian_digraph(n):
    G = nx.DiGraph()
    cycle = list(range(n))
    G.add_edges_from((cycle[i], cycle[(i + 1) % n], {'weight': random.randint(1, 10)}) for i in range(n))
    return G

#Generate num_graphs_per_category graphs per category from categories
#Each graph has num_nodes nodes and the connected graph has num_edges_for_connected edges
def generate_graphs(categories, num_nodes, num_edges_for_connected=None, num_graphs_per_category=1):
    graphs = {}
    for category in categories:
        graphs[category] = []
        for _ in range(num_graphs_per_category):
            if category == "Directed cycle graph":
                graphs[category].append(create_directed_cycle_graph(num_nodes))
            elif category == "Directed tree":
                graphs[category].append(create_directed_tree(num_nodes))
            elif category == "Directed bipartite graph":
                graphs[category].append(create_directed_bipartite_graph(num_nodes // 2, num_nodes // 2))
            elif category == "Directed planar graph":
                graphs[category].append(create_directed_planar_graph(num_nodes))
            elif category == "Directed star graph":
                graphs[category].append(create_directed_star_graph(num_nodes))
            elif category == "Directed strongly connected graph":
                graphs[category].append(create_directed_strongly_connected_graph(num_nodes))
            elif category == "Directed connected graph":
                graphs[category].append(create_directed_connected_graph(num_nodes, num_edges_for_connected))
            elif category == "Directed Hamiltonian-Digraph":
                graphs[category].append(create_directed_hamiltonian_digraph(num_nodes))

    return graphs


#Conert graph into the correct encoding
def describe_graph(G,encoding):
    nodes = list(G.nodes)
    edges = list(G.edges(data=True))
    description = ""
    if encoding=="graph":
        description = f"The directed graph contains the nodes: {', '.join([f'{n}' for n in nodes])}. "
        for u, v, data in G.edges(data=True):
            description += f"There is an edge from {u} to {v} with weight {data['weight']}. "

    elif encoding=="friendship":
        description = f"In a directed friendship network are the persons: {', '.join([f'{n}' for n in nodes])}. "
        for u, v, data in G.edges(data=True):
            description += f"{u} is friends with {v} (strength {data['weight']}). "

    elif encoding == "expert":
        description = f"You are a graph analyst and you have been given a directed graph G among {', '.join(map(str, nodes))}. "
        description += "G has the following directed edges: "
        for u, v, data in edges:
            description += f"{u} -> {v} (weight {data['weight']}), "
        description = description.rstrip(", ") + "."
    
    elif encoding == "politician":
        description = f"G describes a social network graph among {', '.join([f'{n}' for n in nodes])}. "
        description += "We have the following directed connections in G: "
        for u, v, data in edges:
            description += f"{u} follows {v} (weight {data['weight']}). "
    
    elif encoding == "co-authorship":
        description = f"G describes a co-authorship graph among {', '.join([f'{n}' for n in nodes])}. "
        description += "In this co-authorship graph: "
        for u, v, data in edges:
            description += f"{u} wrote a paper that references {v} (weight {data['weight']}). "
    
    elif encoding == "incident":
        description = f"G describes a directed graph among nodes {', '.join([f'{n}' for n in nodes])}. "
        edge_dict = {}
        for u, v, data in edges:
            edge_dict.setdefault(u, []).append(f"{v} (weight {data['weight']})")

        for u in edge_dict:
            outgoing = ', '.join(edge_dict[u])
            description += f"Node {u} has directed edges to nodes {outgoing}. "
...
#Normalize the LLM answer
#Ensures comparison with ground truth
...
#50 possible node names 
names = ["James", "Olivia", "Michael", "Emma", "William", "Ava", "Benjamin", "Sophia", "Lucas", "Isabella", "Mason", "Mia", "Ethan", "Harper", "Elijah", "Charlotte", "Logan", "Amelia", "Jackson", "Evelyn", "Henry", "Abigail", "Alexander", "Ella", "Oliver", "Scarlett", "Daniel", "Lily", "Matthew", "Grace", "Samuel", "Victoria", "Joseph", "Zoey", "David", "Aria", "Carter", "Madison", "Wyatt", "Layla", "Gabriel", "Penelope", "Levi", "Chloe", "Isaac", "Nora", "Sebastian", "Ellie", "Owen", "Stella", "Caleb"]
numbers = list(range(1, 51))
politicians = ["Barack", "Hillary", "Bernie", "Kamala", "Bill", "Arnold", "Jimmy", "Elizabeth", "John", "Angela","Emmanuel", "Justin", "Margaret", "Winston", "Theodore", "Franklin", "Ronald", "Joe", "Donald", "George","Abraham", "Richard", "Lyndon", "Woodrow", "Harry", "Dwight", "Thomas", "Benjamin", "Gerald", "Chancellor","Tony", "David", "Gordon", "Theresa", "Rishi", "Vladimir", "Nicolas", "Silvio", "Matteo", "Ursula","Olaf", "Helmut", "Konrad", "Jacinda", "Pedro", "Felipe", "Recep", "Narendra", "Xi", "Shinzo"]

def getQuestionForIndividualGraph(question,questiondID,encoding,graph):
    ...
    if "Exists a node called <node>?" in question:
        if encoding == 'friendship'or encoding == "co-authorship":
            available_nodes = names 
        elif encoding == 'graph'or encoding=="expert" or encoding=="incident":
            available_nodes = numbers   
        elif encoding == 'politician':
            available_nodes = politicians
        else:
            available_nodes = numbers 
    else:
...
def relabelGraphForEncoding(graph,encoding):

    if(encoding == "friendship" or encoding == "co-authorship"):
        mapping = {node: name for node, name in zip(graph.nodes(), names)}
    elif (encoding=='graph' or encoding=="expert" or encoding=="incident"):
        mapping = {node: name for node, name in zip(graph.nodes(), numbers)}
    elif (encoding=='politician'):
        mapping = {node: name for node, name in zip(graph.nodes(), politicians)}
...
graph_categories = [
    ...
    "Directed Hamiltonian-Digraph"
]

encodings = ["graph", "friendship", "expert", "politician", "co-authorship", "incident"]
...
model_name = "microsoft/phi-4"  # Modellname aktualisieren
...
# Execution
num_nodes = 10
num_edges_for_connected = 15
num_graphs_per_category = 1

graphs_map = generate_graphs(graph_categories, num_nodes, num_edges_for_connected, num_graphs_per_category)

print("Start")
counter = 1
with open("resultsPhi.csv", mode='w', newline='', encoding='utf-8') as file: 
    writer = csv.DictWriter(file, fieldnames=['Graph Category','Graph','Question Category','QuestionID', 'Question','Encoding','Prompting', 'GT', 'Answer','Normalized', 'Correct','Reasoning','Complete prompt'], delimiter=';')
    writer.writeheader()
    for graphCategory, graphs in graphs_map.items():  
        for g, graph in enumerate(graphs, start=1):        
            for encoding in encodings:
                encoded_graph = relabelGraphForEncoding(graph,encoding)
                natural_text = describe_graph(encoded_graph,encoding)
                for questionCategory, questions in questions_map.items():
                    for questiondID, question in questions:
                        ...
                        if "<node>" in question or "<edge>" in question:
                            scale_number_per_individual_question = 1 #questions that can be individualized

                        for scale_numer in range(scale_number_per_individual_question):
                            individualQuestion, ground_truth = getQuestionForIndividualGraph(question,questiondID,encoding,encoded_graph) 
                            ...
                            for prompting in promptings:
                                ...
                                #Create promptText for current styles
                                if prompting!= "zs":
                                    if "role" in parts:
                                    ...
                                    if len(parts) > 3:
                                            raise ValueError(f"Zu viele Teile in parts: {parts}")
                                    elif len(parts) == 0:
                                ...
                                if(sc_prompting_activated):
                                    try:
                                        results = []
                                        for i in range(5):  #Five repetitions for Self-consistency
                                            res = generator(prompt_trimmed)
                                            results.append(res)
                                        normalized_pairs = []
                                        for r in results:
                                            if "answer" in r:
                                                norm = normalize_answer(r["answer"], question, graph, questiondID)
                                ...
                                else: #Prompting Style DOES NOT contain Self-Consistency
                                    try:
                                ...
                                for model, result in evaluation_results.items():
                                    writer.writerow({
                                        ...
                                        'Normalized': result['Normalisiert'],
                                        'Correct': result['Korrekt'],
                                        'Reasoning': reasoning,
